...
{
	if ((KEY_SELECT_GPIO_Port->IDR & KEY_SELECT_Pin) == 0) 
		return 1;
	else 
		return 0;
}
...
{
	if ((KEY_CONFIRM_GPIO_Port->IDR & KEY_CONFIRM_Pin) == 0) 
		return 1;
	else 
		return 0;
}
...
//{
//	//if ((M_KEY_STOP_GPIO_Port->IDR & M_KEY_STOP_Pin) == 0) 
//		return 1;
//	else 
//		return 0;
...
//}
//SERIVE MACHINE KEY 
//static uint8_t IsKeyDown4(void) {if ((KEY_POWER_GPIO_Port->IDR & KEY_POWER_Pin) == 0) return 1;else return 0;}
//static uint8_t IsKeyDown5(void) {if ((KEY_ADD_GPIO_Port->IDR & KEY_ADD_Pin) == 0) return 1;else return 0;}
//static uint8_t IsKeyDown6(void) {if ((KEY_DEC_GPIO_Port ->IDR & KEY_DEC_Pin) == 0) return 1;else return 0;}
...
static void bsp_InitKeyVar(void)
{
	...
	/* 判断按键按下的函数 */
	s_tBtn[0].IsKeyDownFunc = IsKeyDown1; //初始化函数入口，函数指针
	...
	if (pBtn->IsKeyDownFunc())
	{
		...
		else
		{
			if (pBtn->State == 0)
			{
				...
				//s_tBtn[i].KeyCodeUp   = 3 * i + 2;				/* 按键弹起的键值代码 */
				//s_tBtn[i].KeyCodeLong = 3 * i + 3;				/* 按键被持续按下的键值代码 */
				bsp_PutKey((uint8_t)(3 * i + 1));// i=0; key_code=1, i=1,key_code= 4, 短按按键
			...
			if (pBtn->LongTime > 0)
			{
				if (pBtn->LongCount < pBtn->LongTime)
				{
					...
					{
						/* 键值放入按键FIFO */
						bsp_PutKey((uint8_t)(3 * i + 3)); // i=0; key_code=3, i=1,key_code= 6,长按按键
					}
				}
				else
				{
